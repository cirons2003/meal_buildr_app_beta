...
const useSendMessage = () => {
    ...
    const sendMessageWithUser = async(message_text, recipient_id) => {
        ...
        try {
            const response = await axios.post(baseURL.current+'/sendMessage', {message_text: message_text, recipient_id: recipient_id}, {withCredentials: true})
    ...
    const sendMessageWithConversation = async(message_text, conversation_id) => {
        ...
        if (message_text === '')
            return
        try {
            const response = await axios.post(baseURL.current + '/sendMessage', {message_text: message_text, conversation_id: conversation_id}, {withCredentials: true})
            console.log(response.data)
            setCid(response.data.conversation_id)
            setRefreshToggle(!refreshToggle)
        }catch(err) {
